...
JHS1 %>% 
  ...
  geom_point(aes(y = sbp, shape = HSgrad2, fill = HSgrad2, color = HSgrad2), size = 0.5, alpha = 0.45) +
...
|               <font color="#00C5CD" size="4"> [BMI = 32]  ➜ $\phantom0 \hat{sbp} = 103.0857 - 0.9905(x_{HSgrad}) + 0.4376(x_{age})$ </font>

|               <font color="#FF8C00" size="4"> [BMI = 27]  ➜ $\phantom0 \hat{sbp} = 98.6582 - 1.2690(x_{HSgrad}) + 0.5051(x_{age})$ </font>

|               <font color="#BF3EFF" size="4"> [BMI = 25]  ➜ $\phantom0 \hat{sbp} = 96.8872 - 1.3804(x_{HSgrad}) + 0.5321(x_{age})$ </font>
...
JHS1 %>% 
  ggplot(aes(x = age, group = HSgrad2)) +
  geom_point(aes(y = sbp, shape = HSgrad2, fill = HSgrad2, color = HSgrad2), size = 0.8, alpha = 0.5) +
  scale_shape_manual(values = c(24, 21)) +
  scale_fill_manual(values = c("cornsilk2", "cornsilk2")) +
  scale_color_manual(values = c("cornsilk4", "cornsilk4")) +
  geom_line(aes(y = yhat_BMI25), color = "darkorange", linewidth = 0.8, linetype = 1) +
  geom_line(aes(y = yhat_BMI27), color = "darkorchid1", linewidth = 0.8, linetype = 1) +
  geom_line(aes(y = yhat_BMI32), color = "turquoise3", linewidth = 0.8, linetype = 1) +
  labs(x = "Age", y = "Systolic Blood Pressure") +
  facet_wrap(facets = "HSgrad2") +
  ylim(105, 145) +
  theme_bw(base_size = 5.5) +
